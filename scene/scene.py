...
class Scene:
    def __init__(self, ambient_strength: float = 0.4, device: torch.device = torch.device("cpu")):
        self.points = torch.empty((0, 4, 1), device=device)
        self.edges = []
    ...
    def add_point(self, data: torch.Tensor | list):
        ...
        if data.shape[0] == 3:
            ...
            data = torch.cat([data, torch.ones(1, device=self.device)], axis=-1)
        elif data.shape[0] == 4:
    ...
    def add_light(self, direction: list | torch.Tensor, strength: float = 0.5):
        dir = read_3d_vector(direction, device=self.device)
        dir /= torch.linalg.norm(dir)
        self.lights.append((dir, strength))

        return dir
    
    def transform_points(self, R: torch.Tensor, t: torch.Tensor):
        tform = get_4d_transform(R, t)
        self.points = tform.to(self.device) @ self.points